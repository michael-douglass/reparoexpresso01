                      ...
                      >
                        <div className="flex items-start justify-between gap-3">
                          <div className="flex items-start gap-3 min-w-0">
                            {req.provider_id && <ProviderMiniPhoto providerId={req.provider_id} size="sm" />}
                            <div className="min-w-0">
                              <p className="font-semibold text-foreground">{SERVICE_LABELS[req.service_type] || req.service_type}</p>
                              {req.provider_name && (
                                <p className="text-xs text-muted-foreground mt-0.5">Prestador: {req.provider_name}</p>
                              )}
                            </div>
                          </div>
                          <ChevronRight className="w-5 h-5 text-muted-foreground flex-shrink-0 mt-1" />
                        ...
                        {req.description && (
                          <div className="bg-muted/40 rounded-lg p-2.5">
                            <p className="text-xs font-semibold text-muted-foreground mb-1">Descrição do serviço</p>
                            <p className="text-sm text-foreground whitespace-pre-wrap break-words">{req.description}</p>
                          </div>
                        )}
                        ...
                        {req.checklist && (req.checklist.items?.length > 0 || req.checklist.photos?.length > 0 || req.checklist.notes) && (
                          <div className="bg-muted/40 rounded-lg p-2.5 space-y-2">
                            <p className="text-xs font-semibold text-muted-foreground flex items-center gap-1">
                              <CheckSquare className="w-3.5 h-3.5" /> Checklist do serviço
                            </p>
                            {req.checklist.items?.length > 0 && (
                              <div className="space-y-1">
                                {req.checklist.items.map((item, idx) => (
                                  <div key={idx} className="flex items-center gap-2 text-xs">
                                    ...
                                    <span className={cn("text-foreground", !item.checked && "text-muted-foreground line-through")}>
                                      {item.label}
                                    </span>
                                  </div>
                                ))}
                            ...
                            {req.checklist.photos?.length > 0 && (
                              <div className="flex flex-wrap gap-1.5">
                                {req.checklist.photos.map((photo, idx) => (
                                  <a
                                    key={idx}
                                    href={photo}
                                    target="_blank"
                            ...
                            {req.checklist.notes && (
                              <div>
                                <p className="text-xs font-semibold text-muted-foreground mb-0.5 flex items-center gap-1">
                                  <ImageIcon className="w-3 h-3" /> Observações
                                </p>
                                <p className="text-xs text-foreground whitespace-pre-wrap break-words">{req.checklist.notes}</p>
                              </div>
                            )}
                          </div>
                        )}
